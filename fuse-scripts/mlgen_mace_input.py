...
os.environ['GULP_LIB']=''
os.environ['VASP_PP_PATH']="/home/m/msrbzq/VASP-PPs/"
...
os.environ['CONDA']='/home/m/msrbzq/APPS/miniconda3/bin/conda'
os.environ['GNBOSS']='/home/m/msrbzq/APPS/miniconda3/envs/gnboss'
os.environ['GNBOSS_TEMP']='/home/m/msrbzq/APPS/FUSE-stable/fuse_v2/gnboss_template'

os.environ['SPP_PATH']='/home/m/msrbzq/APPS/FUSE-stable/fuse_v2/SPP/'
...
# FUSE setups ##################################################################
composition={'In':2,'Ga':2,'Zn':1,'O':7}, # the emperical foruma unit for this calculation
max_atoms=100, # Maximum number of atoms to use in the overall calculation
imax_atoms=100, # Maximum numbe rof atoms to use in only the inital population, for large systems,it can be helpful to set this smaller than the "max_atoms".
...
initial_gen=100, # the number of structures to include in the initial population
iterations=150, # the number of structures to perform energy calculations on in this run of FUSE.
check_bonds=True,
#dist_cutoff = 1.0,
#density_cutoff=0.9,
pull_random=False, # if set to True, when using pre-built structure pull them in a random order
pull_spp_rank=False, # if True, pull pre-built structures as ranked by spp potentials. BEWARE! you need to first run a script to rank all pre-generated structures & produce one or more csv files containing file names and corresponding energies
...
rank_structures='opti', # if None; do not rank structures, this should only be set if pull_random = True above, if 'opti' rank with SPPs AFTER geometry optimising the, if 'sing' rank based on single point calculations with SPPs.
...
ranking='mace', # if None; do not rank structures, this should only be set if pull_random = True above, if 'opti' rank AFTER geometry optimising the, if 'sing' rank based on single point calculations.
